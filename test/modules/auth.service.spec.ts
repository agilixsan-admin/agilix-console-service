...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('harus mengembalikan accessToken, refreshToken, dan data user saat login berhasil', async () => {
      ...
      const result = await service.login({ email: SUPER_ADMIN_EMAIL, password: TEST_PASSWORD_PLAIN });
      ...
      expect(userRepository.update).toHaveBeenCalledWith(user.id, expect.objectContaining({ lastLoginAt: expect.any(Date) }));
    });
    ...
    it('harus throw UnauthorizedException jika email tidak ditemukan', async () => {
      ...
      await expect(service.login({ email: 'notfound@example.com', password: TEST_PASSWORD_PLAIN }))
    ...
    it('harus throw UnauthorizedException jika password salah', async () => {
      ...
      await expect(service.login({ email: SUPER_ADMIN_EMAIL, password: 'WrongPassword123' }))
        .rejects.toThrow(UnauthorizedException);
    ...
    it('harus throw UnauthorizedException jika user tidak aktif', async () => {
      const hash = await bcrypt.hash(TEST_PASSWORD_PLAIN, 10);
      const user = buildUserWithPassword({ isActive: false, passwordHash: hash });
      userRepository.findByEmailWithPassword.mockResolvedValue(user);

      await expect(service.login({ email: SUPER_ADMIN_EMAIL, password: TEST_PASSWORD_PLAIN }))
        .rejects.toThrow(UnauthorizedException);
  ...
  describe('refresh', () => {
    it('harus mengembalikan accessToken baru jika refresh token valid', async () => {
      ...
      jwtService.verify.mockReturnValue({ sub: TEST_USER_ID, email: SUPER_ADMIN_EMAIL, role: UserRole.FINANCE_ADMIN });
      userRepository.findById.mockResolvedValue(user);
      jwtService.sign.mockReturnValue(TEST_ACCESS_TOKEN);

      const result = await service.refresh({ refreshToken: TEST_REFRESH_TOKEN });
    ...
    it('harus throw UnauthorizedException jika refresh token tidak valid', async () => {
      ...
      await expect(service.refresh({ refreshToken: 'invalid.token' }))
        .rejects.toThrow(UnauthorizedException);
    });
    ...
    it('harus throw UnauthorizedException jika user dari token tidak aktif', async () => {
      ...
      await expect(service.refresh({ refreshToken: TEST_REFRESH_TOKEN }))
        .rejects.toThrow(UnauthorizedException);
    });

    it('harus throw UnauthorizedException jika user dari token tidak ditemukan', async () => {
      jwtService.verify.mockReturnValue({ sub: TEST_USER_ID_NONEXISTENT, email: 'gone@example.com', role: UserRole.FINANCE_ADMIN });
  ...
  describe('getProfile', () => {
    ...
    it('harus throw UnauthorizedException jika user tidak ditemukan', async () => {
      ...
      await expect(service.getProfile(TEST_USER_ID_NONEXISTENT))
        .rejects.toThrow(UnauthorizedException);
    });
  });
});
